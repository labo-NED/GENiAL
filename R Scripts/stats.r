...
# ================= SEX MODERATION: EEG × Sex on Behavior ==============
# Test if associations between theta and behavior vary by sex

# Ensure sex is a factor
if (!is.factor(eeg_behavioral_dataset$sex)) {
  eeg_behavioral_dataset$sex <- as.factor(eeg_behavioral_dataset$sex)
}

# Outcomes to test (SRS + ADHD)
sex_mod_outcomes <- c(
  srs_outcomes,
  "attention_deficit_hyperactivity_tscore"
)

sink("/Users/emmanuelle.coutu-nadeau/Code/NED LAB/GENiAL/R_output/eeg_theta_sex_interaction.txt")
cat("=================================================================\n")
cat("SEX MODERATION: Outcome ~ Theta * Sex + age + IQ + cluster_group\n")
cat("(Interaction term tests whether theta-behavior link differs by sex)\n")
cat("=================================================================\n\n")
...
if (!(theta_feature %in% names(eeg_behavioral_dataset))) {
  ...
  out_rows <- list()
  for (y in sex_mod_outcomes) {
    if (!(y %in% names(eeg_behavioral_dataset))) next
    f <- as.formula(paste(
      y,
      "~",
      paste(c(theta_feature, "sex", paste0(theta_feature, ":sex"), "age", "IQ", "cluster_group"), collapse = " + ")
    ))
    fit <- lm(f, data = eeg_behavioral_dataset)
    sm <- summary(fit)
    rsq <- sm$r.squared
    rsq_a <- sm$adj.r.squared
    nobs <- stats::nobs(fit)
    # Robustness: interaction name depends on factor coding; match by pattern
    cname <- grep(paste0("^", theta_feature, ":sex"), rownames(sm$coefficients), value = TRUE)
    if (length(cname) == 0) cname <- grep(paste0("^sex:", theta_feature), rownames(sm$coefficients), value = TRUE)
    if (length(cname) >= 1) {
      beta_int <- sm$coefficients[cname[1], "Estimate"]
      se_int   <- sm$coefficients[cname[1], "Std. Error"]
      t_int    <- sm$coefficients[cname[1], "t value"]
      p_int    <- sm$coefficients[cname[1], "Pr(>|t|)"]
    } else {
      beta_int <- se_int <- t_int <- p_int <- NA_real_
    }
    out_rows[[length(out_rows) + 1]] <- data.frame(
      Outcome = y,
      Interaction = paste0(theta_feature, ":sex"),
      Beta_int = round(beta_int, 4),
      SE_int = round(se_int, 4),
      t_int = round(t_int, 3),
      p_int = signif(p_int, 3),
      R2 = round(rsq, 3),
      R2_adj = round(rsq_a, 3),
      N = nobs
    )
  }
  if (length(out_rows)) {
    res <- do.call(rbind, out_rows)
    # Multiple-testing correction over outcomes
    res$p_int_FDR <- signif(p.adjust(res$p_int, method = "BH"), 3)
    print(res, row.names = FALSE)
  }
}
sink()

# Stratified trends by sex: simple visualization
if (theta_feature %in% names(eeg_behavioral_dataset)) {
  pdf("/Users/emmanuelle.coutu-nadeau/Code/NED LAB/GENiAL/R_output/plots/theta_behavior_by_sex.pdf", width = 10, height = 8)
  for (y in sex_mod_outcomes) {
    if (!(y %in% names(eeg_behavioral_dataset))) next
    p <- ggplot(eeg_behavioral_dataset, aes(x = .data[[theta_feature]], y = .data[[y]], color = sex)) +
      geom_point(alpha = 0.7) +
      geom_smooth(method = "lm", se = TRUE) +
      labs(
        title = paste("Theta vs", y, "by Sex"),
        x = theta_feature,
        y = y,
        color = "Sex"
      ) +
...
if (!(theta_feature %in% names(eeg_behavioral_dataset))) {
  ...
  for (y in srs_outcomes) {
    ...
    f <- as.formula(paste(
      y,
      "~",
      paste(c(theta_feature, "age", "IQ", "sex", "cluster_group"), collapse = " + ")
    ))
    fit <- lm(f, data = eeg_behavioral_dataset)
    smry <- summary(fit)
