...
# === EXAMPLE COMMANDS ===
# To run with 2s preprocessed data: `python Scripts/feature_extraction.py --hurst --fooof --bandpower --directory /Volumes/NED_Backup3/Q1K_Preprocessed_2s_Happe/5 - processed`
# To run with 5s preprocessed data: `python Scripts/feature_extraction.py --entropy --mse --directory /Volumes/NED_Backup3/Q1K_Preprocessed_5s_Happe/5 - processed`
...
for sub, fname in enumerate(file_names):
    names.append(fname)
    raw = mne.io.read_raw_eeglab(os.path.join(args.directory, fname), preload=True)
    data = raw.get_data()  # shape: (n_channels, n_times)
    Fs = int(raw.info['sfreq'])
    n_channels = data.shape[0]
    n_epochs = 1  # If you have epochs, adjust accordingly

    # If you have epochs, use mne.Epochs and loop over epochs
