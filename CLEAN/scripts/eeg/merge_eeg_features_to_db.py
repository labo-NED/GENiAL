...
def main():
    print("=" * 80)
    print("EEG Features and Clustered Data Merger")
    print("=" * 80)
    ...
    # Read the clustered data file
    print(f"\n3. Reading clustered data from:\n   {CLUSTERED_DATA_FILE}")
    clustered_df = pd.read_csv(CLUSTERED_DATA_FILE)
    print(f"   - Total rows: {len(clustered_df)}")
    print(f"   - Columns: {len(clustered_df.columns)}")
    print(f"   - Unique participants: {clustered_df['participant_id'].nunique()}")
    
    # Merge the dataframes on participant_id
    print(f"\n4. Merging datasets on 'participant_id'")
    merged_df = clustered_df.merge(eeg_rsrio, on='participant_id', how='left')
    ...
    output_filename = f"merged_clustered_EEG_features_global_RSRio_{timestamp}.csv"
    output_path = os.path.join(OUTPUT_DIR, output_filename)
    
    # Save the merged dataframe
    print(f"\n5. Saving merged data to:\n   {output_path}")
    ...
    print(f"  - Clustered data: {len(clustered_df)} rows, {len(clustered_df.columns)} columns")
    print(f"\nOutput file:")
